...
class XAVATARModel(pl.LightningModule):
    ...
    def plot(self, data, res=128, verbose=True, fast_mode=False,calc_time=False):
        ...
        if verbose:
            ...
            smpl_verts = self.smpl_server.verts_c if fast_mode else data[
                'smpl_verts'][[0]]
            surf_pred_def, time = self.extract_mesh(
                smpl_verts,
                data['smpl_tfs'][[0]],
                data['smpl_thetas'][[0]],
                data['smpl_exps'][[0]]
                ...
                canonical=False,
                with_weights=False,
                fast_mode=fast_mode,calc_time=calc_time)
            img_pred_def = render_trimesh(surf_pred_def, mode='t')
            results = {'img_all': img_pred_def, 'mesh_def': surf_pred_def,'times':time}
    ...
    def extract_mesh(self,
                     ...
                     with_weights=False,
                     res_up=2,
                     fast_mode=False,calc_time=False):
        '''
        ...
        no_cond = torch.zeros_like(cond).to(cond.device)
        ...
        if calc_time:
            end_color.record()
            torch.cuda.synchronize()
            time['color_time'] = start_color.elapsed_time(end_color)
