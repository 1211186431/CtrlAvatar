...
from lib.model.cond_net import condNet
import copy
...
class XAVATARModel(pl.LightningModule):
    ...
    def __init__(self, opt, meta_info, data_processor=None):
        ...
        self.deformer = ForwardDeformer(opt.deformer,
                                        ...
                                        d_out=d_out)
        self.delta_net = ImplicitNetwork(**opt.deltanet)
    ...
    def configure_optimizers(self):
        params = [
            ...
            },
            {
                ...
                'params': self.delta_net.parameters()
            },{
                'params': self.cond_net.parameters()
            }
        ]
        if self.train_color:
            params.append({
    ...
    def forward(self,
        ...
        ## cond emb
        pts_c_emb = self.cond_net(pts_c,cond)
        ## delta
        pts_c_delta = self.delta_net(pts_c_emb,return_feature=False)
        pts_c = pts_c + pts_c_delta*0.1
        no_cond = torch.zeros_like(cond)
        
        # query shape in canonical space
        shape_pd = self.shape_net(pts_c, no_cond, return_feature=False)
    ...
    def plot(self, data, res=128, verbose=True, fast_mode=False,is_test=False,calc_time=False):
        ...
        if verbose:
            ...
            surf_pred_def, pre_t_mesh ,times= self.extract_mesh(
                smpl_verts,
                data['smpl_tfs'][[0]],
                data['smpl_thetas'][[0]],
                data['smpl_exps'][[0]]
                ...
                canonical=False,
                with_weights=False,
                fast_mode=fast_mode,is_test=is_test,calc_time=calc_time)

            # img_pred_def = render_trimesh(surf_pred_def, mode='t')
            results = {'img_all': None, 'mesh_def': surf_pred_def,'t_mesh':pre_t_mesh,'times':times}
    ...
    def extract_mesh(self,
                     ...
                     with_weights=False,
                     res_up=2,
                     fast_mode=False,is_test=False,calc_time=False):
        '''
        ...
        if calc_time:
            time = dict()
            start_mesh = torch.cuda.Event(enable_timing=True)
            end_mesh = torch.cuda.Event(enable_timing=True)
            
            start_deformer = torch.cuda.Event(enable_timing=True)
            end_deformer = torch.cuda.Event(enable_timing=True)
        cond = torch.cat(
            ...
            def cano_forward(xc, cond, shape_net, color_net, with_color=False):
                ...
                for xc_split in torch.split(xc, batch_points, dim=1):
                    ...
                    shape_pd, pose_cond, feature = shape_net(
                        xc_split, no_cond, return_feature=True)
                    accum_shape.append(shape_pd)
                    if with_color:
        ...
        if not hasattr(self, 'mesh') and is_test:
            if calc_time:
                start_mesh = torch.cuda.Event(enable_timing=True)
                start_mesh.record()  # 记录某个特定操作的开始时间   
            mesh = generate_mesh(
                func,
            ...
            if calc_time:
                ...
                time['mesh_time'] = start_mesh.elapsed_time(end_mesh)
            ...
            mesh = generate_mesh(
                ...
                shape_represenation=self.opt.network.representation)
        if calc_time:
            start_deformer.record()
        with torch.no_grad():
            mesh_verts = torch.tensor(mesh.vertices).type_as(smpl_verts)
            pts_c_emb = self.cond_net(mesh_verts.unsqueeze(0),cond)
            all_delta = self.delta_net(pts_c_emb,return_feature=False).squeeze(0)
            mesh_verts = mesh_verts - all_delta*0.1
            mesh.vertices = mesh_verts.cpu().numpy()
        ...
        if calc_time:
            end_deformer.record() 
            torch.cuda.synchronize()
            time['deformer_time'] = start_deformer.elapsed_time(end_deformer)
        if with_weights:
