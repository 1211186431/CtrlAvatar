...
def generate_mesh(func, verts, level_set=0, res_init=32, res_up=3, shape_represenation='occ'):

    scale = 1.3  # Scale of the padded bbox regarding the tight one.
    ...
    with torch.no_grad():
        while points.shape[0] != 0:
            ...
            _, values, _, _ = func(points.unsqueeze(0))
            values = values.reshape(-1) #(num_point,)
