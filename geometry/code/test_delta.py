...
from lib.xavatar_scan_model_delta import XAVATARSCANModel
...
def main(opt):
    ...
    for i in trange(smpl_params_all.shape[0]):
        ...
        results = model.plot(data,
                             ...
                             fast_mode=opt.demo.fast_mode,is_test=True,calc_time=True)
        for key in times.keys():
            times[key]+= results['times'][key]
        if i == 0:
            results['t_mesh'].export("t_mesh.ply")
